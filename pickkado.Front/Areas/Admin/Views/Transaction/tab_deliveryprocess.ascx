<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<pickkado.Front.Areas.Admin.Models.DeliveryProcessTransactionFilterViewModel>" %>


<%var pagination = ViewBag.Pagination; %>
<div style="padding:10px">
    <%using (Html.BeginForm("tab_deliveryprocess", "transaction", FormMethod.Post, new {@class="form-inline" , id="onDeliveryProcessForm"})) {%>    
        <div class="row">
            ...
            <div class="form-group">
                <a class="btn btn-primary" href="javascript:SearchDelivery()" >Search</a>
            </div>
    ...
    <tbody>
            <%foreach (var item in ViewBag.DeliveringList)
                ...
                <tr class="animated">
                    ...
                    <td style="vertical-align:middle;"><%:item.ResiNumber %></td>
...
<script>
    function SearchDelivery() {
        $.post('/admin/transaction/tab_deliveryprocess', $('#onDeliveryProcessForm').serialize(), function (data) {
            if (data)
                $('#deliverprocesslist .card-body').html(data);
        });
    }
</script>