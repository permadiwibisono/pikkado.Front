<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<pickkado.Front.Areas.Vendor.Models.PopupConfirmOrderViewModel>" %>

<div class="modal-content">
    <% using (Html.BeginForm("popup_confirm_delivery", "popup", FormMethod.Post, new { @class = "form-horizontal", id = "confirmForm" }))
   { %>
<%: Html.AntiForgeryToken()%>
                ...
                <div class="container-fluid form-horizontal">
                    ...
                    <div class="row" style="padding:2px 0;">
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <textarea class="form-control" disabled rows="3"><%:Model.AlamatPickkado %></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row" style="padding:2px 0;">
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" value="<%:Model.NamaKurir %>"  disabled />
                            </div>
                        </div>
                    </div>
                    <div class="row" style="padding:2px 0;">
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" value="<%:Model.JenisPaket %>"  disabled />
                            </div>
                        </div>
                    </div>
                    <div class="row" style="padding:2px 0;">
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                ...
                                <%:Html.HiddenFor(m=>m.JenisPaket) %>
                                <%:Html.HiddenFor(m=>m.AlamatPickkado) %>
                                <%:Html.TextBoxFor(m=>m.NoResi,new{@class="form-control",placeholder="Nomor resi*"}) %>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $('#Save').click(function () {
            $.post($('#confirmForm').attr('action'), $('#confirmForm').serialize())
                .done(function (data) {
                    if (data.IsError) {
                        if (data.HtmlString) {
                        ...
                        '</div>';
                        $('#message').html(message);
                        $('#readylist table tr[data-bind="' + $('#Id').val() + '"').addClass('fadeOutRight');
                        setTimeout(function () {
                            $('#readylist table tr[data-bind="' + $('#Id').val() + '"').remove();
