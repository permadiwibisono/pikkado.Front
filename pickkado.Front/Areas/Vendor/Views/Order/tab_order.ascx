    ...
    <tbody>
        <%foreach (var item in (List<pickkado.Front.Areas.Vendor.Models.OrderViewModel>)ViewBag.List) {
          ...
          <tr class="animated" data-bind="<%:item.Id%>">
              ...
              <td>
                  <div class="btn-group">
                      <button class="btn btn-success" type="button" onclick="Accept('<%:item.Id %>')">Accept</button>
                      <button class="btn btn-danger" type="button" onclick="Reject('<%:item.Id %>')">Reject</button>
