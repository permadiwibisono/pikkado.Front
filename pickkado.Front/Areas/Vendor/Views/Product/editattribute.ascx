<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<pickkado.Front.Areas.Vendor.Models.EditAttributeViewModel>" %>

<div class="modal-content">
        
<% using (Html.BeginForm("editattribute", "product", new {id=ViewContext.RouteData.Values["id"] }, FormMethod.Post, new { @class = "form-horizontal", @enctype = "multipart/form-data", id="newAttributeForm" }))
   { %>
    ...
    <div class="modal-body">
       ...
       foreach (var i in ViewBag.MasterList)
       {
           list.Add(new SelectListItem
           {
               Text = i.Name,
               Value = i.Id,
               Selected=Model.AttributeId==i.Id?true:false
           });
       }
                    %>
            ...
            success: function (data) {
                ...
                if (data.IsError == true) {
                    ...
                    var row = '<td>' + data.Obj.Type + '</td>' +
                                ...
                                '</form>' +
                            '</td>';
                    var tr = $('#EDIT' + data.Obj.Id).parents('tr:first');
                    tr.html(row);
                    tr.removeClass('fadeIn');
                    tr.addClass('fadeInLeft');
                }
            },
            error: function (err) {
