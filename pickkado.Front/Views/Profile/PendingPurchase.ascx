...
<div style="padding: 50px; margin-left: 120px">
    <div class="container-fluid title-underline">
        <span class="font-avant" style="font-size: 15px">Pembelian yang Ditunda
        </span>
    </div>
    <%var userLogin = (pickkado.Entities.User)TempData["UserLogin"]; %>
    <%var pendingModel = Model.Where(a => (a.Status == pickkado.Front.TransactionStatus.InventoryChecking ||
                                            a.Status == pickkado.Front.TransactionStatus.PaymentChecking ||
                                            a.Status == pickkado.Front.TransactionStatus.WaitingForYourFriendPayment ||
                                            a.Status == pickkado.Front.TransactionStatus.NotValid ||
                                        a.Status == pickkado.Front.TransactionStatus.UnderPayment) &&
            ...
            <div>
                ...
                <div id="div<%:m.Id %>" class="collapse" >
                    ...
                    <div>
                        <table>
                            <tr>
                                <%if (m.Status != pickkado.Front.TransactionStatus.PaymentChecking &&
                                      m.Status != pickkado.Front.TransactionStatus.WaitingForYourFriendPayment &&
                                      m.Status != pickkado.Front.TransactionStatus.InventoryChecking)
                                  { %>
    ...
    <% } else {%>
        <div style="background-color:#F5F7F6; text-align:center; padding:20px; margin-top:5px" class="font-avant">
            Tidak ada pembelian yang ditunda
        </div>
    <% } %>
